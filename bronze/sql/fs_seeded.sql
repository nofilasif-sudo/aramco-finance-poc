...
-- bronze.fs_seeded
--
-- GENERATED from bronze_ingest.cloud.BRONZE_TABLES['fs_seeded'] — the same
...
--
-- Source: Lynn's flat extract PoC_Group_FS_seeded.csv, read by
...
OPTIONS(
  description="Saudi Aramco Group condensed consolidated financial statements for Q1 2026, the SEEDED-ERROR version, loaded verbatim from Lynn's flat extract. 142 rows = income statement 19 lines x 2 periods + balance sheet 38 x 2 + Notes 5/7/10 7+5+7 x 1 column + Note 9 3 x 3 columns. Structurally identical to fs_clean — the defects change three values, never the shape. THE THREE PLANTED DEFECTS: (1) Total current assets at 31 Mar 2026 prints 683,180 where its eight components foot to 682,760, a 420 break that also breaks the balance sheet, since Total assets still prints 2,661,959; (2) Revenue from contracts with customers in Note 10 prints 423,221 where its three components foot to 423,218, a 3 break; (3) non-current Borrowings cites note 5 where the borrowings note is 9 — a text-only defect with no arithmetic signal, detectable only by validating the reference against the note. Defects 1 and 2 are in amount, defect 3 is in note_ref. TYPED, not all-STRING: amount NUMERIC, line_order INT64 — the only such columns in bronze, so that footing checks run at zero tolerance. Sum with a line_role = 'item' filter; totals and subtotals share the table with their components. No source_file column: the table name and doc_version identify the document. fs_clean and fs_seeded share this schema and have NO relationship — no join keys, no shared surrogate ids, and nothing presuming the two documents correspond row-for-row; the agent reads both and performs its own comparison. Ingest checks structure only and NEVER refuses to land on an arithmetic break — the breaks are the point. SYNTHETIC data."
);
