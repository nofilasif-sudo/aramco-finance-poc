...
--
-- Covers exactly what silver_build.sql touches: dim_entity, dim_period,
-- dim_account, dim_group_account, fact_trial_balance,
...
checks AS (
  ...
  -- 1. dim_entity now carries all three entities, Aramco flagged as the group.
  SELECT 1 AS seq, 'dim_entity has 3 rows (2010, 2380, ARAMCO)' AS check_name,
         COUNT(*) = 3 AS ok, FORMAT('actual %d', COUNT(*)) AS detail FROM de
  UNION ALL
  SELECT 2, 'exactly one entity is_group = true',
         COUNTIF(is_group) = 1, FORMAT('is_group count %d', COUNTIF(is_group)) FROM de
  UNION ALL
  SELECT 3, 'ARAMCO is the is_group entity',
         (SELECT is_group FROM de WHERE entity_code = 'ARAMCO') = TRUE,
         'n/a' FROM (SELECT 1)

  -- 2. dim_period shape.
  UNION ALL
  SELECT 4, 'dim_period has 9 rows, sort_order dense 1..9',
         COUNT(*) = 9 AND MIN(sort_order) = 1 AND MAX(sort_order) = 9
           AND COUNT(DISTINCT sort_order) = 9,
         FORMAT('actual %d, distinct sort_order %d', COUNT(*), COUNT(DISTINCT sort_order)) FROM dp
  UNION ALL
  SELECT 5, 'period_key matches year/quarter',
         COUNTIF(period_key != CONCAT(CAST(year AS STRING), 'Q', CAST(quarter AS STRING))) = 0,
         FORMAT('mismatched %d', COUNTIF(period_key != CONCAT(CAST(year AS STRING), 'Q', CAST(quarter AS STRING)))) FROM dp

  -- 3. dim_account / dim_group_account unaffected by the switch to MERGE —
  --    same shape checks as before.
  UNION ALL
  SELECT 6, 'dim_account has 110 rows (66 + 44)',
         COUNT(*) = 110, FORMAT('actual %d', COUNT(*)) FROM da
  UNION ALL
  SELECT 7, 'dim_account (entity_code, account_code) unique',
  ...
  UNION ALL
  SELECT 8, 'dim_group_account has 78 rows, group_node unique',
         COUNT(*) = 78 AND COUNT(*) = COUNT(DISTINCT group_node),
         FORMAT('actual %d, distinct %d', COUNT(*), COUNT(DISTINCT group_node)) FROM dga

  -- 3b. fact_trial_balance — rewired onto bronze_tb_raw this pass. Grain,
  --     referential integrity via the COMPOSITE key, and the strongest
  --     check available: double-entry proves to nil, independently per
  --     entity per period.
  UNION ALL
  SELECT 9, 'fact_trial_balance has 990 rows (110 accounts x 9 periods)',
         COUNT(*) = 990, FORMAT('actual %d', COUNT(*)) FROM ftb
  UNION ALL
  SELECT 10, 'fact_trial_balance grain is unique',
         COUNT(*) = COUNT(DISTINCT FORMAT('%s|%s|%s', entity_code, account_code, period_key)),
         FORMAT('%d rows, %d distinct keys', COUNT(*),
                COUNT(DISTINCT FORMAT('%s|%s|%s', entity_code, account_code, period_key))) FROM ftb
  UNION ALL
  SELECT 11, 'every fact_trial_balance row joins to dim_account on (entity_code, account_code)',
         COUNTIF(d.account_code IS NULL) = 0,
         FORMAT('orphan rows %d', COUNTIF(d.account_code IS NULL))
  FROM ftb f LEFT JOIN da d USING (entity_code, account_code)
  UNION ALL
  SELECT 12, 'fact_trial_balance source_file never blank',
         COUNTIF(COALESCE(source_file, '') = '') = 0,
         FORMAT('blank %d', COUNTIF(COALESCE(source_file, '') = '')) FROM ftb
  UNION ALL
  SELECT 13, 'fact_trial_balance proves to nil per entity per period',
         COUNTIF(residual != 0) = 0,
         FORMAT('non-zero entity-periods %d', COUNTIF(residual != 0))
  FROM (SELECT entity_code, period_key, SUM(ledger_amount) AS residual
        FROM ftb GROUP BY entity_code, period_key)

  -- 4. fact_group_trial_balance — new table. Grain, referential integrity,
  --    and the strongest check available: it proves to nil independently
  --    of the affiliate side.
  UNION ALL
  SELECT 14, 'fact_group_trial_balance has 450 rows (50 nodes x 9 periods)',
         COUNT(*) = 450, FORMAT('actual %d', COUNT(*)) FROM fgtb
  UNION ALL
  SELECT 15, 'fact_group_trial_balance grain is unique',
  ...
  UNION ALL
  SELECT 16, 'every fact_group_trial_balance row joins to dim_group_account',
  ...
  UNION ALL
  SELECT 17, 'every fact_group_trial_balance row joins to dim_entity',
  ...
  UNION ALL
  SELECT 18, 'fact_group_trial_balance proves to nil per period (Aramco parent-only)',
         COUNTIF(residual != 0) = 0,
  ...
  -- 5. Agent reference data.
  UNION ALL
  SELECT 19, 'dim_ifrs_requirement has 15 rows (3 standards x 5 reqs)',
         COUNT(*) = 15, FORMAT('actual %d', COUNT(*)) FROM dir
  UNION ALL
  SELECT 20, 'dim_ifrs_requirement (standard_code, req) unique',
  ...
  UNION ALL
  SELECT 21, 'dim_ifrs_requirement standard_code fully populated (backfill took)',
  ...
  UNION ALL
  SELECT 22, 'dim_ifrs_requirement evidence_type is a closed set',
  ...
  UNION ALL
  SELECT 26, 'every dim_ifrs_requirement resolves to dim_ifrs_standard',
  ...
  UNION ALL
  SELECT 27, 'no dim_ifrs_standard row is unused',
