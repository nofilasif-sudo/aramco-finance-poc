...
def read_excel_from_gcs(storage_client: storage.Client, gcs_uri: str) -> pd.ExcelFile:
    ...
    file, which BytesIO provides directly. Writing to /tmp first was never
    actually required.
    """
...
def run_sql_file(bq_client: bigquery.Client, sql_path: str | Path) -> int:
    ...
    silver MERGE scripts (dim_entity, dim_period, fact_trial_balance)."""
...
def validate_zero_sum(bq_client: bigquery.Client, fact_table_id: str, tolerance: float = 0.01) -> None:
    """Enforce the fact table's core invariant: ledger_amount must sum to
    zero per entity/period. Raises if any group is out of balance, so the
    pipeline fails loudly instead of silently shipping wrong numbers.
    """
    query = f"""
        SELECT entity_code, period_key, SUM(ledger_amount) AS total
        FROM `{fact_table_id}`
        GROUP BY entity_code, period_key
        HAVING ABS(total) > {tolerance}
    """
    logger.info("Validating zero-sum invariant on %s ...", fact_table_id)
    rows = list(bq_client.query(query).result())

    if rows:
        details = ", ".join(f"{r.entity_code}/{r.period_key}={r.total}" for r in rows)
        raise ValueError(f"{fact_table_id}: {len(rows)} entity/period group(s) do not sum to zero: {details}")

    logger.info("Zero-sum check passed for %s", fact_table_id)