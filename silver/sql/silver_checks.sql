...
-- dim_account, dim_group_account, fact_group_trial_balance,
-- dim_ifrs_standard, dim_ifrs_requirement, dim_entity_context,
-- dim_required_document.
--
-- fact_trial_balance is OUT OF SCOPE — sourced from bronze_tb_raw, owned by
-- another developer's trial_balance/ pipeline, same as silver_build.sql.
--
...
checks AS (
  ...
  UNION ALL
  SELECT 9, 'fact_group_trial_balance has 450 rows (50 nodes x 9 periods)',
         COUNT(*) = 450, FORMAT('actual %d', COUNT(*)) FROM fgtb
  UNION ALL
  SELECT 10, 'fact_group_trial_balance grain is unique',
  ...
  UNION ALL
  SELECT 11, 'every fact_group_trial_balance row joins to dim_group_account',
  ...
  UNION ALL
  SELECT 12, 'every fact_group_trial_balance row joins to dim_entity',
  ...
  UNION ALL
  SELECT 13, 'fact_group_trial_balance proves to nil per period (Aramco parent-only)',
  ...
  UNION ALL
  SELECT 14, 'dim_ifrs_requirement has 15 rows (3 standards x 5 reqs)',
         COUNT(*) = 15, FORMAT('actual %d', COUNT(*)) FROM dir
  UNION ALL
  SELECT 15, 'dim_ifrs_requirement (standard_code, req) unique',
  ...
  UNION ALL
  SELECT 16, 'dim_ifrs_requirement standard_code fully populated (backfill took)',
  ...
  UNION ALL
  SELECT 17, 'dim_ifrs_requirement evidence_type is a closed set',
  ...
  UNION ALL
  SELECT 21, 'every dim_ifrs_requirement resolves to dim_ifrs_standard',
  ...
  UNION ALL
  SELECT 22, 'no dim_ifrs_standard row is unused',
